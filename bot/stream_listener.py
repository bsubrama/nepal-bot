...
class CustomStreamListener(tweepy.StreamListener):
  ...
  def on_status(self, status):
    ...
    if (status.user.screen_name not in apis.keys() and
      ...
      mc.set(key, status)
      self.scheduled += 1
      logger.log(' '.join(['publishing tweet_id', str(status.id),
                           ' from ', status.user.screen_name,
                           ' matching ', ','.join([word for word in config.CONFIG['keywords']  if word in status.text]),
                           ' as tweet_num:', str(self.scheduled),
                           ' to RabbitMQ']))
      #queue.put((self.scheduled, status))
      channel.basic_publish(exchange='',
                            routing_key=config.CONFIG['rabbitmq_config']['queue'],
                            body=pickle.dumps((self.scheduled, status)),
